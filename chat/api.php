...
if ($action === 'messages') {
    ...
    if (!$convId || !isMember($db, $convId, $userId)) {
        jsonOut(['ok' => false, 'error' => 'Không có quyền']);
    }
...
if ($action === 'send') {
    ...
    if (!$convId || $content === '') {
        jsonOut(['ok' => false, 'error' => 'Thiếu dữ liệu']);
    }
    if (mb_strlen($content) > 4000) {
        jsonOut(['ok' => false, 'error' => 'Tin nhắn quá dài']);
    }
    if (!isMember($db, $convId, $userId)) {
        jsonOut(['ok' => false, 'error' => 'Không có quyền']);
    }

    $db->prepare('INSERT INTO messages (conversation_id, user_id, content) VALUES (?, ?, ?)')
        ->execute([$convId, $userId, $content]);
    $msgId = (int) $db->lastInsertId();

    $stmt = $db->prepare("
        SELECT m.id, m.content, m.created_at, m.user_id, u.display_name, u.username
        FROM messages m JOIN users u ON u.id = m.user_id WHERE m.id = ?
    ");
    $stmt->execute([$msgId]);
    jsonOut(['ok' => true, 'message' => $stmt->fetch()]);
}
...
if ($action === 'create_group') {
    ...
    if ($name === '' || mb_strlen($name) > 80) {
        jsonOut(['ok' => false, 'error' => 'Tên nhóm không hợp lệ']);
    }
    if (!is_array($memberIds)) $memberIds = [];
    $memberIds = array_unique(array_map('intval', $memberIds));
    $memberIds = array_filter($memberIds, fn($id) => $id > 0 && $id !== $userId);

    $db->beginTransaction();
    try {
        $db->prepare('INSERT INTO conversations (type, name, description, created_by, is_private) VALUES (?, ?, ?, ?, 1)')
            ->execute(['group', $name, $description ?: null, $userId]);
        $convId = (int) $db->lastInsertId();

        $db->prepare('INSERT INTO conversation_members (conversation_id, user_id, role) VALUES (?, ?, ?)')
            ->execute([$convId, $userId, 'admin']);
        ...
        $db->rollBack();
        jsonOut(['ok' => false, 'error' => 'Không tạo được nhóm']);
    }
}
...
if ($action === 'start_dm') {
    ...
    if ($otherId <= 0 || $otherId === $userId) {
        jsonOut(['ok' => false, 'error' => 'User không hợp lệ']);
    }
    $check = $db->prepare('SELECT id FROM users WHERE id = ?');
    $check->execute([$otherId]);
    if (!$check->fetch()) {
        jsonOut(['ok' => false, 'error' => 'Không tìm thấy người dùng']);
    }
    ...
    $existing = $stmt->fetch();
    if ($existing) {
        jsonOut(['ok' => true, 'conversation_id' => (int) $existing['id']]);
    }

    $db->beginTransaction();
    try {
        $db->prepare('INSERT INTO conversations (type, name, created_by, is_private) VALUES (?, NULL, ?, 1)')
            ->execute(['dm', $userId]);
        $convId = (int) $db->lastInsertId();
        $db->prepare('INSERT INTO conversation_members (conversation_id, user_id, role) VALUES (?, ?, ?)')
            ->execute([$convId, $userId, 'member']);
        $db->prepare('INSERT INTO conversation_members (conversation_id, user_id, role) VALUES (?, ?, ?)')
            ->execute([$convId, $otherId, 'member']);
        $db->commit();
        jsonOut(['ok' => true, 'conversation_id' => $convId]);
    } catch (Exception $e) {
        $db->rollBack();
        jsonOut(['ok' => false, 'error' => 'Không tạo được cuộc trò chuyện']);
    }
}
...
if ($action === 'info') {
    ...
    if ($conv['type'] === 'dm') {
        foreach ($conv['members'] as $m) {
            if ((int)$m['id'] !== $userId) {
                $conv['name'] = $m['display_name'];
                break;
            }
        }
    }
    jsonOut(['ok' => true, 'conversation' => $conv]);
}
