...
if (!$tag) {
    ...
    echo '<div class="container"><div class="empty-state"><h3>Tag không tồn tại</h3><p><a href="index.php">Về trang chủ</a></p></div></div>';
...
$stmt = $db->prepare("SELECT COUNT(*) FROM posts p JOIN post_tags pt ON p.id = pt.post_id WHERE pt.tag_id = ? AND p.status = 'published'");
$stmt->execute([$tag['id']]);
...
$pageDesc = 'Bài viết gắn tag ' . $tag['name'];
require __DIR__ . '/includes/header.php';
?>
...
<div class="container">
    ...
    <?php if (empty($posts)): ?>
        <div class="empty-state">
            <h3>Chưa có bài viết với tag này</h3>
