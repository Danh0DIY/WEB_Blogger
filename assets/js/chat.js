...
(function () {
    ...
    async function openConversation(id) {
        ...
        if (info.ok) {
            ...
            $('#chatSubtitle').textContent = c.type === 'dm'
                ? 'Tin nhắn riêng'
                : n + ' thành viên · Nhóm riêng tư';
        }
    ...
    function toggleMember(id, user) {
        if (selectedMembers.has(id)) {
            ...
            selectedMembers.set(id, user);
        }
        renderSelected();
        $$('.user-pick', $('#groupUserList')).forEach(b => {
            b.classList.toggle('selected', selectedMembers.has(+b.dataset.id));
        });
    }
    ...
    function renderSelected() {
        ...
        $$('[data-rm]', el).forEach(b => {
            b.addEventListener('click', () => {
                selectedMembers.delete(+b.dataset.rm);
                renderSelected();
            });
        });
    }
        ...
        const membersHtml = (c.members || []).map(m => `
            <div class="info-member">
                ...
                <span>
                    ...
                    <small>@${esc(m.username)} ${m.role === 'admin' ? '· Admin' : ''}</small>
                </span>
            </div>
        `).join('');
        $('#infoBody').innerHTML = `
            <p class="muted" style="margin-bottom:1rem;">
                ${c.type === 'dm' ? 'Tin nhắn riêng tư' : 'Nhóm chat riêng tư'} · Tạo ${timeFmt(c.created_at)}
            </p>
